...
johnson_klaeger_data = read_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/validation/johnson_klaeger_data.csv'))
```
...
```{r}
#join in johnson data, leaving out WHIM12 data for validation

johnson_data_for_ml = johnson_klaeger_data %>%
  filter(cell_line != "WHIM12") %>% 
  rename(CELLNAME = cell_line) %>% 
  left_join(sample_info %>% 
              select(DepMap_ID, stripped_cell_line_name),
            by = c('CELLNAME' = 'stripped_cell_line_name')) %>%
  mutate(PANEL = "Breast Cancer") %>% 
  select(PANEL, CELLNAME, DepMap_ID, drug1, drug2, CONC1, CONC2, nearest_klaeger_dose_1, nearest_klaeger_dose_2, viability, starts_with("act_"))

ALMANAC_klaeger_johnson_data = ALMANAC_klaeger_data %>% 
  bind_rows(johnson_data_for_ml) %>% 
  drop_na()

 johnson_data_for_ml_combos = johnson_data_for_ml %>% 
  select(drug1, drug2) %>% 
  unique()
```
...
ALMANAC_klaeger_johnson_CCLE_data = ALMANAC_klaeger_johnson_data %>% 
...
this_dataset = ALMANAC_klaeger_johnson_CCLE_data %>% 
	filter(viability <=1)
```

```{r}

write_rds(this_dataset, here('results/ALMANAC_klaeger_johnson_data_for_ml.rds.gz'), compress = "gz")
```
...
```{r}
#write out breast cancer data

breast_cancer_data = this_dataset %>% 
  filter(PANEL == "Breast Cancer") %>% 
  write_rds(here('results/ALMANAC_klaeger_johnson_models/breast_cancer_models/ALMANAC_klaeger_johnson_data_for_ml_breast.rds.gz'), compress = "gz")

write_csv(breast_cancer_data, here('results/ALMANAC_klaeger_johnson_models/breast_cancer_models/ALMANAC_klaeger_johnson_data_for_ml_breast.csv'))
```
