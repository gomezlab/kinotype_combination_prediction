...
validation_klaeger_data =  all_validation_data_filtered %>%
	...
	select(-klaeger_conc) %>%
  ungroup() %>% 
  group_by(drug1, drug2, cell_line, nearest_klaeger_dose_1, nearest_klaeger_dose_2) %>% 
  summarise(viability = mean(viability)) %>% 
...
WHIM12_exp_data = read_rds(here('data/Synergy_data_collection/usable_data/sam_usable/RNAseq/WHIM12_RNAseq_data_for_pan_cancer_model.rds')) %>% 
...
CCLE_data = read_rds(here('data/full_CCLE_expression_set_for_ML.rds')) %>% 
  select(DepMap_ID, any_of(lasso_selected_features$feature)) %>% 
  left_join(cell_info)
...
validation_klaeger_exp_data = validation_klaeger_data %>% 
  ...
  write_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/validation/validation_klaeger_exp_data_for_pan_cancer_model.csv'))
```
