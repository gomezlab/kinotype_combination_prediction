...
visualization_data = breast_cancer_data %>% 
  ...
  group_by(subtype, drug1, drug2, CONC1, CONC2) %>% 
  summarise(viability = mean(viability)) %>% 
...
visualization_data_processed = visualization_data %>% 
  ungroup() %>% 
  mutate(combo = fct_relevel(combo, visualization_data_summary %>% pull(combo)))
...
visualization_data_processed %>% 
  ...
  scale_fill_viridis_d() +
	...
	labs(title = " ",x = "Cell Viability", y = "Kinase Inhibitor Combinations", fill = "Breast Cancer Subtype") +
