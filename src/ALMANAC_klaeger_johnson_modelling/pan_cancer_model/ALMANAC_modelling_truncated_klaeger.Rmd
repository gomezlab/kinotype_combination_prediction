...
write_csv(results, here('results/ALMANAC_klaeger_johnson_models/pan_cancer_models/ALMANAC_klaeger_johnson_lasso_xgboost_all_tuning_results_truncated.csv'))
```

```{r}
#fit model on whole data to get importances
ALMANAC_klaeger_johnson_data = read_csv(here('results/ALMANAC_klaeger_models/ALMANAC_klaeger_data_for_ml_truncated.csv'))

results = read_csv(here('results/ALMANAC_klaeger_johnson_models/pan_cancer_models/ALMANAC_klaeger_johnson_lasso_xgboost_all_tuning_results_truncated.csv'))
...
final_fit = 
	final_wflow %>% 
	fit(ALMANAC_klaeger_johnson_data)
...
write_csv(here('results/ALMANAC_klaeger_johnson_models/pan_cancer_models/lasso_selected_features_truncated.csv'))
```

```{r}
# fit best model with CV for figure
lasso_selected_features = read_csv(here('results/ALMANAC_klaeger_johnson_models/pan_cancer_models/lasso_selected_features_truncated.csv'))
...
id_vars = ALMANAC_klaeger_johnson_data %>% 
  select(-starts_with("act_"),
         -starts_with("exp_"),
         -starts_with("viability")) %>% 
  names()

model_dataset = ALMANAC_klaeger_johnson_data %>% 
...
ggsave(here('figures/ALMANAC_klaeger_johnson/ALMANAC_klaeger_johnson_best_pan_cancer_model_CV_truncated.png'))
```
...
model_dataset = ALMANAC_klaeger_johnson_data %>% 
  select(any_of(c(id_vars, lasso_selected_features$feature, "viability"))) %>% 
  write_rds(here('results/ALMANAC_klaeger_johnson_models/pan_cancer_models/ALMANAC_klaeger_johnson_model_dataset_truncated.rds.gz'))
...
final_fit = 
	this_wflow %>% 
	fit(ALMANAC_klaeger_johnson_data) %>% 
  write_rds(here('results/ALMANAC_klaeger_johnsnon_models/pan_cancer_models/ALMANAC_klaeger_johnson_lasso_xgboost_truncated_final_model.rds.gz'), compress = "gz")
```
