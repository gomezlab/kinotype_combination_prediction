...
single_predictions_drug_1 = All_preds %>%
  ...
  select(drug1, CONC1, DepMap_ID, CELLNAME, predicted_viability) %>% 
  group_by(drug1, CONC1, DepMap_ID, CELLNAME) %>% 
  summarise(mean_predicted_viability = mean(predicted_viability)) %>% 
  ungroup() %>% 
  #impute zeros
  bind_rows(
  All_preds %>% 
	select(drug1, DepMap_ID, CELLNAME) %>% 
	unique() %>% 
	mutate(CONC1 = 0) %>% 
...
single_predictions_drug_2 = All_preds %>%
  ...
  select(drug2, CONC2, DepMap_ID, CELLNAME, predicted_viability) %>% 
  group_by(drug2, CONC2, DepMap_ID, CELLNAME) %>% 
  summarise(mean_predicted_viability = mean(predicted_viability)) %>% 
  ungroup() %>% 
  #impute zeros
  bind_rows(
  All_preds %>% 
	select(drug2, DepMap_ID, CELLNAME) %>% 
	unique() %>% 
	mutate(CONC2 = 0) %>% 
	mutate(mean_predicted_viability = 1)
  )
...
full_predictions = All_preds %>% 
  select(-.pred, -nearest_klaeger_dose_1, -nearest_klaeger_dose_2, -PANEL) %>% 
...
```{r}
#read in processed predictions
full_predictions_summary = read_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/processed_model_predictions.csv'))
