---
title: "Build model predicitons on non-tested data using pan-cancer model"
...
validation_data_for_model = read_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/validation/validation_klaeger_exp_data_for_pan_cancer_model.csv'))
final_model = read_rds(here('results/ALMANAC_klaeger_models/ALMANAC_klaeger_lasso_xgboost_truncated_final_model.rds.gz'))
```
...
not_tested_data = validation_data_for_model %>%
  ...
  select(DepMap_ID, drug1, drug2, CONC1, CONC2, cell_line, viability, starts_with(c("act_", "exp"))) %>% 
...
model_predictions_tidy = model_predictions %>%
  ...
  select(-.pred) %>% 
	write_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/model_predictions_for_validation_pan_cancer_model.csv'))
```
...
ggsave(here('figures/ALMANAC_klaeger/breast_cancer_individual_combo_validation_assessment_pan_cancer_model.png'), height = 20, width = 30, units = "cm")
```
...
ggsave(here('figures/ALMANAC_klaeger/breast_cancer_individual_cell_line_validation_assessment_pan_cancer_model.png'), height = 15, width = 15, units = "cm")
