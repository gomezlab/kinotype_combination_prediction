...
lasso_selected_features = read_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/lasso_selected_features_all_kinases_forced_in.csv'))

validation_data_for_model = read_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/validation/validation_klaeger_exp_data_for_all_kinases_forced_in.csv'))
final_model = read_rds(here('results/ALMANAC_klaeger_models/breast_cancer_models/final_model_all_kinases_forced_in.rds.gz'))
```

```{r}
already_tested_combos = ALMANAC_klaeger_CCLE_data %>% 
...
not_tested_data = validation_data_for_model %>%
  ...
  rename(CONC1 = nearest_klaeger_dose_1, CONC2 = nearest_klaeger_dose_2) %>% 
  select(DepMap_ID, drug1, drug2, CONC1, CONC2, cell_line, viability, starts_with("act_"), any_of(lasso_selected_features$feature)) %>% 
...
model_predictions_tidy = model_predictions %>%
  ...
  select(-.pred) %>% 
	write_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/model_predictions_for_validation_all_kinases_forced_in.csv'))
```
...
ggsave(here('figures/ALMANAC_klaeger/breast_cancer_individual_combo_validation_assessment_all_kinases_forced_in.png'), height = 20, width = 30, units = "cm")
```
...
model_predictions_with_metrics = model_predictions_tidy %>% 
  ...
  facet_wrap(~cell_line + this_rsq)

ggsave(here('figures/ALMANAC_klaeger/breast_cancer_individual_cell_line_validation_assessment_all_kinases_forced_in.png'), height = 10, width = 10, units = "cm")
