---
title: "Modelling Breast Cancer Cell Lines including all kinase features"
...
this_recipe = recipe(ALMANAC_klaeger_CCLE_data) %>%
  ...
  step_zv(all_predictors()) %>% 
	step_select_linear(starts_with("exp"), outcome = "viability", engine = "glmnet", penalty = tune(), mixture = tune(), top_p = tune())
...
write_csv(results, here('results/ALMANAC_klaeger_models/breast_cancer_models/ALMANAC_klaeger_lasso_xgboost_all_tuning_results_all_kinases_forced_in.csv'))
```

```{r}
#fit model on whole data to get lasso feature coefficients 

results = read_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/ALMANAC_klaeger_lasso_xgboost_all_tuning_results_all_kinases_forced_in.csv'))
...
write_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/lasso_selected_features_all_kinases_forced_in.csv'))
```
...
ggsave(here('figures/ALMANAC_klaeger/breast_ALMANAC_klaeger_best_model_CV.png'), width = 7, height = 8, units = "cm")
```
...
write_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/feature_importances_all_kinases_forced_in.csv'))
```
...
final_fit = 
	final_wflow %>% 
	fit(model_dataset) %>% 
  write_rds(here('results/ALMANAC_klaeger_models/breast_cancer_models/final_model_all_kinases_forced_in.rds.gz'), compress = "gz")
