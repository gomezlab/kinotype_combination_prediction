...
lasso_selected_features = read_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/lasso_selected_features.csv')) %>% 
  ...
  filter(Importance > 0)
validation_data_for_model = read_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/validation/validation_klaeger_exp_data.csv'))
...
already_tested_combos = ALMANAC_klaeger_CCLE_data %>% 
  filter(CELLNAME == "MDA-MB-231/ATCC") %>% 
  select(drug1, drug2, CELLNAME) %>% 
  unique() %>% 
  mutate(cell_line = "MDAMB231")
...
not_tested_data = validation_data_for_model %>%
  ...
  select(DepMap_ID, drug1, drug2, CONC1, CONC2, cell_line, viability, any_of(lasso_selected_features$feature)) %>% 
...
model_predictions_tidy = model_predictions %>%
  ...
  select(-.pred) %>% 
	write_csv(here('results/ALMANAC_klaeger_models/breast_cancer_models/model_predictions_for_validation.csv'))
```
...
ggsave(here('figures/ALMANAC_klaeger/breast_cancer_individual_combo_validation_assessment.png'), height = 20, width = 30, units = "cm")
```
...
ggsave(here('figures/ALMANAC_klaeger/breast_cancer_individual_cell_line_validation_assessment.png'), height = 10, width = 10, units = "cm")
